...
public class BarCreator : MonoBehaviour
{
    ...
    bool IsPointInsideTerrain(Vector2 pos)
    {
        int terrainMask = LayerMask.GetMask("Terrain");
        return Physics2D.OverlapCircle(pos, 0.15f, terrainMask) != null;
    }

    bool IsAnchoredNodeAtPos(Vector2 pos)
    {
        foreach (var p in nodes)
        {
            if (p == null) continue;
            if (Vector2.Distance(p.transform.position, pos) < 0.05f && p.isAnchored)
                return true;
        }
        return false;
    }
    ...
    {
        if (IsPointInsideTerrain(pos))
        {
            if (IsAnchoredNodeAtPos(pos))
            {
                foreach (var p in nodes)
                    if (p != null && Vector2.Distance(p.transform.position, pos) < 0.05f)
                        return p;
            }
